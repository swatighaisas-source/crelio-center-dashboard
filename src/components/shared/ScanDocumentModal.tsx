...
type Screen = "scan-document" | "pdf-preview";

interface Props {
  open: boolean;
  scanner: ScannerDevice | null;
  onClose: () => void;
  onConfirm: (fileName: string) => void;
}

export function ScanDocumentModal({ open, scanner, onClose, onConfirm }: Props) {
  const [screen, setScreen] = useState<Screen>("scan-document");
  ...
  async function handleScan() {
    ...
    try {
      ...
      const { fileName } = await simulateScannerCapture(device);
  ...
  const modelName = scanner?.modelName ?? "Ambir ImageScan Pro 490i";
  const title = screen === "scan-document" ? `Scan Document (${modelName})` : "Scan Document";

  return (
    <>
      <div className="bill-attach-overlay" style={{ zIndex: 1200 }} onClick={handleClose}>
        <div
          className="bill-attach-modal"
          role="dialog"
          aria-modal="true"
          aria-label={title}
          style={{ maxWidth: screen === "pdf-preview" ? 960 : 700 }}
          onClick={(e) => e.stopPropagation()}
        >
          <div className="bill-attach-modal-header">
            <span className="bill-attach-modal-title">{title}</span>
              ...
              <div className="bill-attach-body bill-attach-body--scan">
                ...
                <div className="bill-attach-scan-controls">
                  <div className="bill-attach-scan-controls-left">
                    ...
                    >
                      {SCANNER_UPLOAD_FILE_TYPES.map((t) => <option key={t}>{t}</option>)}
                    </select>
                    <button
          ...
          {screen === "pdf-preview" && (
            <>
              ...
              <div className="bill-attach-footer bill-attach-footer--preview">
                ...
                <button
                  className="bill-attach-btn bill-attach-btn--teal"
                  onClick={() => { onConfirm(scannedFileName); handleClose(); }}
                >
                  Confirm and Upload
                </button>
              </div>
      ...
      {scanSettingsOpen && (
        <div className="bill-attach-settings-overlay" style={{ zIndex: 1300 }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="bill-attach-body">
              <div className="bill-attach-settings-grid">
                <div className="bill-attach-settings-field">
                  ...
                  <select value={draftSettings.resolutionDpi} onChange={(e) => setDraftSettings((s) => ({ ...s, resolutionDpi: e.target.value }))}>
                    {SCANNER_RESOLUTION_OPTIONS.map((o) => <option key={o}>{o}</option>)}
                  </select>
                </div>
                <div className="bill-attach-settings-field">
                  <label>Color Mode</label>
                  <select value={draftSettings.colorMode} onChange={(e) => setDraftSettings((s) => ({ ...s, colorMode: e.target.value }))}>
                    {SCANNER_COLOR_MODE_OPTIONS.map((o) => <option key={o}>{o}</option>)}
                  </select>
                </div>
                <div className="bill-attach-settings-field">
                  <label>Paper Size</label>
                  <select value={draftSettings.paperSize} onChange={(e) => setDraftSettings((s) => ({ ...s, paperSize: e.target.value }))}>
              ...
              <div className="bill-attach-toggles-row">
                <label className="bill-attach-toggle-item">
                  <input type="checkbox" checked={draftSettings.autoCrop} onChange={(e) => setDraftSettings((s) => ({ ...s, autoCrop: e.target.checked }))} />
                  Auto Crop
                </label>
                <label className="bill-attach-toggle-item">
                  <input type="checkbox" checked={draftSettings.autoDeskew} onChange={(e) => setDraftSettings((s) => ({ ...s, autoDeskew: e.target.checked }))} />
                  Auto Deskew
                </label>
                <label className="bill-attach-toggle-item">
                  <input type="checkbox" checked={draftSettings.autoRotate} onChange={(e) => setDraftSettings((s) => ({ ...s, autoRotate: e.target.checked }))} />
                  Auto Rotate
                </label>
              </div>
            </div>
